...
try {
    ...
    $secretsDir = Join-Path "{{ WORK_DIR }}" "secrets"
    if (Test-Path $secretsDir) {
        Get-ChildItem "$secretsDir\*.env" | ForEach-Object {
    ...
    $result = & claude -p "/{{ SKILL_PREFIX }}-heartbeat" `
        --max-turns 30 `
    ...
    $ingestScript = Join-Path "{{ WORK_DIR }}" "search\ingest.py"
    if (Test-Path $ingestScript) {
        $venvPython = Join-Path "{{ WORK_DIR }}" "tools\venv\Scripts\python.exe"
        if (Test-Path $venvPython) {
            $reportsDir = Join-Path "{{ WORK_DIR }}" "reports"
            $notesDir = Join-Path "{{ WORK_DIR }}" "notes"
            $entriesDir = Join-Path "{{ WORK_DIR }}" "blog\entries"
            & $venvPython $ingestScript $reportsDir $notesDir $entriesDir --no-embed --quiet 2>$null
        }
    }
} finally {
    Remove-Item $LockFile -Force -ErrorAction SilentlyContinue
